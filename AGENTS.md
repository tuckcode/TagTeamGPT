...
- Workspace MCP (`mcp/`) plus `tools/lob-*` give Chat optional read-only repo eyes via an HTTPS tunnel — not the Chat↔Codex bridge (that’s keybinds/clipboard/mailbox); paste-back works without MCP; mailbox is not on the Chat MCP surface. ChatGPT connector name is `codexgpt-workspace` (distinct from the old Lob-gpt skill/plugin); auth is header `Authorization` = `Bearer` + hex from `.lob/mcp.token` (leave the Bearer token env var / `MCP_BEARER_TOKEN` blank — it is a placeholder). Settings “Connected” ≠ enabled on a new Chat thread — enable in the + menu. A new tunnel URL does not rebind the connector — edit/reload connector settings when the URL changes.
