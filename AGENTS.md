...
- Public product name is **Lob** (tagline: Chat lobs. Codex dunks.). Skill id and GitHub repo are still `$codexgpt` / `tuckcode/CodexGPT` until renamed. Desktop-app take on XiaoDuoYa/codex-with-chatgpt: Chat plans and reviews; Codex executes.
...
- Shared agent skills canonicalize under `~/.agents/skills`; the in-repo Lob skill is `.agents/skills/codexgpt` and is invoked in ChatGPT desktop Codex with `$codexgpt` (or “Use Lob to …”) — `$` is not a Cursor skill picker (`/` is).
