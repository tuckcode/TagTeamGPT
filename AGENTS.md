...
- Prefer the agent to verify or drive desktop workflows when possible instead of asking the user to try them manually (user should not become the copy-paster); do not report Codex as executing from driver paste/mode alone — confirm the intended named Chat/Codex session is focused and working.
...
- Never re-press Chat or Codex mode hotkeys (macOS ⌃1/⌃3) when already in that mode — Chat’s ⌃1 can open New chat and abandon the pinned planner thread; paste with `send` / `chat-send` / `codex-send` without forcing the mode key.
...
- For CodexGPT, the product loop runs in ChatGPT desktop (Chat plans, Codex executes); Cursor/this agent is build-chat only (driver/loop/MCP tooling) — not product glue — and must not “prove” the loop by writing kata files here.
